/**
 File Name: main.js
 File Author: Sam James <sam@samdjames.uk>
 File Desc: Handle the implementation details of CS3Dv2 on steam marketplace pages
 */
...
const app = {
    ...
    logPluginInfoBanner: function ()
    {
        console.log.apply(console, [
            `%c CS3D %cv${config.version} by SamJ %c http://www.samdjames.uk/CS3D`, 
            "background: #000000;color: #00ff99", 
    ...
    extractItemData: function ()
    {
        ...
        if (item === null) {
            return logger.error('Issue extracting item string info');
        }
        ...
        if(itemArray.length <= 1) {
            return logger.error('Error Getting name & skin');
        }

        this.itemName = itemArray[0].trim();
        this.skinName = itemArray[1].trim();

        logger.debug(`Weapon - ${this.itemName}`);
    ...
    weaponExists: function (item, skin)
    {
        if(!config.items.hasOwnProperty(item)) {
            logger.error('That weapon does not exists');
            return false;
        }

        if(!config.items[item].textures.includes(skin)) {
            logger.error(`Skin does not exists for ${item}`);
            return false;
        }
    ...
    addHTMLToPage: function ()
    {
        ...
        if(image === null) {
            return logger.error('Could not find the image to insert the toggle into');
        }
        ...
        if (toggle === null) {
            return logger.error('Could not find the toggle to setup');
        }
    ...
    {
        logger.debug('Handle Toggle View');
    ...
    loadCS3D: function ()
    {
        ...
        if(canvas === null) {
            return logger.error(`Render is undefined`);
        }
